...
class Admin_Ticketlog extends MY_Controller {
	...
	function logs()
	{
		$config['base_url'] = site_url('admin_ticketlog/logs');
        $config['total_rows'] = $this->model_ticketing->count_alltickets();
        $config['per_page'] =  10;
        $config['num_links'] = 5;
        ...
        $data['loglinks'] = $this->pagination->create_links();
        ...
        $data['log'] = $this->model_ticketing->get_alltickets($config['per_page'], $this->uri->segment(3));
   	    $this->template->load('admin_template', 'view_adminticketlog', $data);
	}
